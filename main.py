...
## 為什麼45度比較好，我覺得SSIM測的怪怪的不然我也沒辦法了。
m = 0
m_ang = symmetric_angle_list[0][0]
for i in range(10):
    ...
    if(ang == 45):
        cv2.imwrite('r.png',right_side)
        cv2.imwrite('l.png',left_side)
    if(ang == 90):
        cv2.imwrite('r2.png',right_side)
        cv2.imwrite('l2.png',left_side)
...
# cv2.imshow('l',left_side)
# cv2.imshow('r',right_side)
...
for i in range(5):
    ...
    x1 = int(center_x + length * math.cos(m_ang / radian))
    y1 = int(center_y - length * math.sin(m_ang / radian))
    x2 = int(center_x - length * math.cos(m_ang / radian))
    y2 = int(center_y + length * math.sin(m_ang / radian))
